...
| --- | --- | --- | --- |
| Prior pinned stock Hermes, v0.20.4 (2026.8.18) | `277268d83ff2204de9646f0b1e376e73ac0a5d20` | 26 | Preserve prior pin; final exact-distribution rerun required |
| Stock Hermes v2026.9.11 | `939e45c91d751fadd94dcd1b873ac3cb44846213` | 30 | Loader/hooks/schema inspected; adapter and real lifecycle acceptance still required |
...
- `hermes_state_common.py:213,308–398`: schema 30 has parent_session_id, profile_name, cwd/Git metadata and archived session flags; messages retain role/content/tool IDs/timestamps/compacted state and add fields including active/effect_disposition. Similar columns do not authorize blindly allowing schema 30. The existing importer rejects unsupported versions until an explicit tested adapter is integrated.
...
`/afterforge` and `/fixlab` share status, scan, failures, review and help. Guided queue/detail/accept/reject/retract behavior is under integration and must be checked against installed help. `fixlab_build_regression` historically registers an existing test file; do not describe that registration as generated code. A concrete draft requires a separate generated artifact with source references, expected failure, frozen inputs and unknowns, followed by explicit review.

`hermes afterforge` and `hermes fixlab` share setup, doctor, scan, serve, export, current-check-plan, verify-current, policy and uninstall-runtime. Bundled skill identity is `agent-fix-lab:regression-workflow`; the older editable skill is separate and not silently removed. See [legacy skill integration](hermes-integration.md).
