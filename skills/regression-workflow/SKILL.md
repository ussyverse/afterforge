...
Inspect a local recipe and both implementations before fixlab_build_regression. The legacy name registers an EXISTING pytest assertion as UNREVIEWED; it does not generate a draft and the model cannot declare review. The backend register_regression operation is its honest alias. Ask the operator to inspect the recipe and run `hermes afterforge review-recipe RECIPE_ID --approve-digest DIGEST` (digest is in the registration response); fixlab_verify_regression refuses unreviewed recipes. Fail/pass/inconclusive/not-run are distinct. A setup failure, skip or missing test cannot verify a fix. Use fixlab_report for a compact report.
