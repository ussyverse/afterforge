...
Native integration backend operation names (coordinator must register host commands; do not invent available model tools): review_queue, review_correction, draft_regression, authorize_draft, guided_case, synthetic_demo, register_regression, retained_plan, retained_check. review_correction defaults to reviewer=agent; an operator terminal action must explicitly set reviewer=operator. The existing review_case remains agent-proposed. review_queue and list_cases accept offset/limit; standalone corrections and list expose those same SQL-bounded pages.
