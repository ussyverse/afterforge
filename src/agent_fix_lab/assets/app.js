...
async function detail(id) {
  ...
  $('recipe-form').onsubmit=e=>{e.preventDefault();const body={reviewed:$('reviewed').checked,recipe:{case_id:id,repository:$('repo').value,faulty_revision:$('faulty').value,corrected_revision:$('corrected').value,test_file:$('test-file').value,expected_behavior:$('recipe-expected').value,intended_failure:$('failure').value}};task('recipe',async()=>{const saved=await api('/api/recipes',body);await detail(id);if(selected===id)$('recipes').value=saved.id;},true);};
...
$('import').onclick=()=>task('import',async()=>{await api('/api/import',{limit:100,after:Date.now()/1000-3600});await list();},true);
task('list',list);
